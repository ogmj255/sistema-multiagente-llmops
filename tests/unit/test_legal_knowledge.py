...
def create_legal_document() -> LegalDocument:
    ...
    return LegalDocument(
        ...
        processed_path=(
            "data/processed/legal/ec_test_law.json"
        ),
    )
...
def test_prepare_legal_document() -> None:
    ...
    assert all(
        chunk.document_id == "ec_test_law"
        for chunk in chunks
    )
    assert all(
        chunk.jurisdiction == "ecuador"
        for chunk in chunks
    )
    assert all(
        chunk.topics == "protección de datos"
        for chunk in chunks
    )
...
def test_prepare_chunks_continues_after_error(
    ...
    valid_path = (
        processed_directory / "valid.json"
    )
    ...
    invalid_path = (
        processed_directory / "invalid.json"
    )
    ...
    assert json.loads(lines[0])["document_id"] == (
        "ec_test_law"
    )